...
class PaymentRepository:
    ...
    async def get_by_idempotency_key(self, async_session: AsyncSession, idempotency_key: str) -> Payment | None:
        stmt = select(Payment).where(Payment.idempotency_key == idempotency_key)
        result = await async_session.execute(stmt)
        return result.scalar_one_or_none()

    async def create_payment(self, async_session: AsyncSession, payment: Payment) -> None:
